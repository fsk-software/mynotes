...
/**
 * Test the {@link com.fsk.mynotes.presentation.animations.filter_toolbar.ToolbarAnimatorBuilder}.
 */
public class ToolbarAnimatorBuilderTest extends ActivityUnitTestCase<MockActivity> {

    public ToolbarAnimatorBuilderTest() {
    ...
    public void testBuildingWithoutSettingAnyValues() {
        try {
            new ToolbarAnimatorBuilder().build();
            assert false;
        }
    ...
    public void testBuildingWithNullTarget() {
        try {
            new ToolbarAnimatorBuilder().setFocus(0, 0).setRadius(1).setTarget(null).build();
            assert false;
        }
        catch (NullPointerException e) {
            assert true;
    ...
    public void testBuildingWithInvalidRadius() {
        try {
            new ToolbarAnimatorBuilder().setFocus(0, 0).setRadius(-1).setTarget(getActivity().mView)
                                        .build();
            assert false;
        }
    ...
    public void testBuildingHideAnimatorSunnyDay() {
        ...
        ToolbarAnimatorBuilder builder =
                new ToolbarAnimatorBuilder().setFocus(-1, -2).setRadius(1).setTarget(target);
...
//        assertEquals(1, animator.getListeners().size());
    ...
    public void testBuildingRevealAnimatorSunnyDay() {
        View target = getActivity().mView;
        ToolbarAnimatorBuilder builder =
                new ToolbarAnimatorBuilder().setFocus(1, 2).setRadius(3).setReveal(true)
                                            .setTarget(target);
        assertEquals(1, builder.mX);
        assertEquals(2, builder.mY);
